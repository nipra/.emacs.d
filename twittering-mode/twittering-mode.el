...
(if twittering-mode-map
    (let ((km twittering-mode-map))
      ...
      (define-key km (kbd "v") 'twittering-other-user-timeline) ; User at the twit status
      (define-key km (kbd "V") 'twittering-visit-timeline)	; Some user's
    ...
    (unless api
      (error "Invalid `twittering-tinyurl-service'. try one of %s"
	     (mapconcat (lambda (x)
			...
			twittering-tinyurl-services-map ", ")))
    (if longurl
	(let* ((longurl-encoded (w3m-url-encode-string longurl))
	       (buffer
		(twittering-url-retrieve-synchronously (concat api longurl-encoded))))
	  (with-current-buffer buffer
	    (goto-char (point-min))
	    (prog1
